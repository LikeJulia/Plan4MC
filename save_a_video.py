...
def image_caption(image,text):
    ...
    color = (255, 0, 0)
    cv2.putText(img=image, text=text, org=(10, 20),
                fontFace=font, fontScale=0.4, thickness=1, color=color)
    return image
    ...
    for ep in range(args.test_episode):
        ...
        if not args.progressive_search:
            ...
            while True:
                ...
                for i_ in range(len(env.rgb_list)-len(skill_list)):
                    skill_list.append(skill_next)
                if task_done:
                    break
        ...
        if args.save_gif and task_success:
            rgb_list = [image_caption(im, text) for (im, text) in zip(env.rgb_list, skill_list)]
            imageio.mimsave(os.path.join(save_dir,'episode{}_success{}.gif'.format(ep,int(task_success))), rgb_list, duration=0.05)
        # save snapshots
...
if __name__ == "__main__":
    ...
    parser.add_argument('--progressive-search', type=int, default=1) # re-compute skill sequence after each skill execution?
    ...
    parser.add_argument('--save-path', type=str, default='save_test_video')
