...
class SelfImitationBuffer:
    ...
    def eval_and_store(self, obs, act, ret, success, rgb=None):
        '''
        ...
        if self.cur_size > 0:
            ...
            self.baseline = np.mean(self.ret_buf) 
...
'''
def ppo_selfimitate_clip_lk(args, seed=0, device=None, 
    ...
    # def image_caption(image,reward):
    #     font = cv2.FONT_HERSHEY_SIMPLEX
    #     color = (255, 255, 0)
    #     action_str7 = 'Reward: '
    #     reward=round(reward, 2)
    #     action_str7 += str(reward)
    #     cv2.putText(img=image, text=action_str7, org=(10, 90),
    #                 fontFace=font, fontScale=0.3, thickness=1, color=color)
    #     return image
        ...
        for t in range(local_steps_per_epoch):
            ...
            if terminal or epoch_ended:
                ...
                if args.task != "creative:12":
                    ...
                    ep_rewards_clip = np.asarray([0 for _ in range(len(rgb_list))])
